...
class P3DLysosomeManager {
   ...
   * starting at descentY depth.
   */
  spawnForChunk(descentY, pH) {
    ...
    // Probabilistic antibody straggler
  ...
  cull(vehicleWorldY) {
    ...
    for (let i = this._lysos.length - 1; i >= 0; i--) {
      if (this._lysos[i].posY > threshold) {
        this._scene.remove(this._lysos[i].group);
        this._lysos.splice(i, 1);
        dodged++;
      }
    }
  ...
  destroy() {
    for (const l of this._lysos) this._scene.remove(l.group);
